...
namespace PrepodPortal.Controllers
{
   public class NewTeacherController : Controller
   {
      ...
      private void sendEmail(string name, string email, string password)
      {
         ...
         mail.Body = string.Format("Вітаємо, {0}. Вас заререєстрував на Prepod Portal працівник кафедри. Ваші дані для входу : 1)Ім'я користувача: {1} ; 2)Пароль: {2} http://pn.sadlogic.com", name, email, password);
         try
         {
